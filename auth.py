...
def activate():
    ...
    try:
        ...
        if request.method == ' GET': 
            ...
            db = get_db()
            attempt = db.execute(
                "SELECT * FROM activatiolink WHERE challenge=? and state=? and CURRENT_TIMESTAMP BETWEEN created and validuntil", (number, utils.U_UNCONFIRMED)
            ).fetchone()
...
def confirm():
    try:
        ...
        if request.method == ?: 
            ...
            if attempt is not None:
                db.execute(
                    #ACTUALIZAR EL REGSTRO EN FORGOTLINK Y CAMBIAR EL ESTADO POR INACTIVO
                    UPDATE, (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    QUERY, (hashP, salt, attempt['userid'])##ACTUALIZAR EL REGISTRO DEL USUIARIO Y CAMBIAR LA CONTRASEÑA Y EL SALT
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
